...
static int ml_kem_pairwise_test(const ML_KEM_KEY *key)
{
    ...
    /*
     * Hash of private key storage (sans "d", which may not be set) is the
     * encapsulation entropy source.
     */
    if ((mdctx = EVP_MD_CTX_new()) == NULL
        || !EVP_DigestInit_ex(mdctx, key->shake256_md, NULL)
        || !EVP_DigestUpdate(mdctx, (unsigned char *)key->t,
                             key->vinfo->prvalloc - ML_KEM_RANDOM_BYTES)
        || !EVP_DigestFinalXOF(mdctx, entropy, sizeof(entropy))) {
