...
operatrion for a raw public key. It behaves similarly to X509_STORE_CTX_init().
...
I<ctx> that contains the constructed (output) chain. It may be NULL if a
raw public key is being validated.

X509_STORE_CTX_get0_rpk() returns the internal pointer used by the
I<ctx> that contains the raw public key. It may be NULL if a certificate
is being validated.
...
The X509_STORE_CTX_init_rpk() function was added in OpenSSL 3.2.
