...
The B<mtype> argument specifies the hash method to use.
...
The B<mtype> argument to SSL_add_expected_rpk() may be one of the following:

=over 4

=item DANETLS_MATCHING_FULL

Use the full value DER value of the public keys for comparison (i.e. no hash).

=item DANETLS_MATCHING_2256

Use the SHA256 hash of the public keys for comparison.

=item DANETLS_MATCHING_2512
