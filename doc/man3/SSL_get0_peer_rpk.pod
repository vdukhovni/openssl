...
SSL_add_expected_rpk() adds public key B<rpk> into the SSL B<s>'s DANE TLSA database.
The B<nid> argument specifies the hash method to use.

SSL_get_negotiated_client_cert_type() returns the connection's negotiated client certificate
type.

SSL_get_negotiated_server_cert_type() returns the connection's negotiated server certificate
type.
...
must be able to accept either one.
...
to verify. They can, however, be matched verbatum of by their digest value, this
is done by specifying one or more TLA records, see L<SSL_CTX_dane_enable(3)>.
...
The SSL_add_expected_rpk() function is a wrapper around L<SSL_dane_tlsa_add(3)>. If DANE
...
=item OSSL_RPK_MATCH_FULL

Use the full value DER value of the public keys for comparison (i.e. no hash).

=item OSSL_RPK_MATCH_SHA256

Use the SHA256 hash of the public keys for comparison.

=item OSSL_RPK_MATCH_SHA512
