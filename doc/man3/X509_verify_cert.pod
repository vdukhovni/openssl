...
The X509_verify_rpk() function validates a raw public key based on parameters
in I<ctx>. This behaves similar to X509_verify_cert(), except that the number
of possible checks is significantly reduced. The raw public key is authenticated
only via DANE TLSA records. See L<SSL_CTX_dane_enable(3)>.
...
X509_verify_rpk() returns 1 if a raw public key can be validated, otherwise
it returns 0, and in exceptional circumstances (such as malloc
